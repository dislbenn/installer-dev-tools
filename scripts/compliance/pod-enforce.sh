...
function try_find_owners {
    ...
    local namespaces=$(yq "keys | .[]" < lint.yaml -oy)
...
function patch_owners {
    local namespaces=$(yq "keys | .[]" < enforce.yaml -oy)
    for namespace in $namespaces; do
        local deployments=$(yq ".$namespace.deployments | .[]" < enforce.yaml -oy)
        for deployment in $deployments; do
            patch_owner $namespace $deployment "deployment"
        done

        local statefulsets=$(yq ".$namespace.statefulsets | .[]" < enforce.yaml -oy)
        for ss in $statefulsets; do
            patch_owner $namespace $ss "statefulset"
        done

        local jobs=$(yq ".$namespace.jobs | .[]" < enforce.yaml -oy)
        for job in $jobs; do
